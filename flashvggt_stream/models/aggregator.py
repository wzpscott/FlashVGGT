...
from flashvggt_stream.layers import PatchEmbed
from flashvggt_stream.layers.block import Block
from flashvggt_stream.layers.rope import RotaryPositionEmbedding2D, PositionGetter
from flashvggt_stream.layers.vision_transformer import vit_small, vit_base, vit_large, vit_giant2
...
class Aggregator(nn.Module):
    ...
    def forward(self, images: torch.Tensor, is_first_chunk: bool = True, memory_drop_rate: int = 1, kv_cache_list=None) -> Tuple[List[torch.Tensor], int, Any]:
        ...
        camera_token = slice_expand_and_flatten(self.camera_token, B, S, is_first_chunk)
        register_token = slice_expand_and_flatten(self.register_token, B, S, is_first_chunk)
...
def slice_expand_and_flatten(token_tensor, B, S, is_first_chunk=True):
    """
    Processes specialized tokens with shape (1, 2, X, C) for multi-frame processing:
    1) Uses the first position (index=0) for the first frame only (if is_first_chunk is True)
    ...
    if is_first_chunk:
        ...
        # Slice out the "other" tokens for all S frames => shape (1, S, ...)
